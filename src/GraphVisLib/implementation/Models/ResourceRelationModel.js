...
export default class ResourceRelationModel extends BaseModel {
    ...
    addResource = resource => {
        if (!this.__isInMap__(this.resourceMap, this.__resourceIdentifier__(resource))) {
            this.__integrateResourceItem(this.modelAsJsonObject.resources, this.resourceMap, resource);
        } else {
            // Has seen this resource >>> Merge <<<;
            this.__mergeItem(this.resourceMap[this.__resourceIdentifier__(resource)], resource);
        }
    };
    ...
    addRelation = relation => {
        if (!this.__isInMap__(this.relationMap, this.__resourceIdentifier__(relation))) {
            this.__integrateResourceItem(this.modelAsJsonObject.relations, this.relationMap, relation);
        } else {
            // Has seen this relation >>> Merge <<<;

            this.__mergeItem(this.relationMap[this.__resourceIdentifier__(relation)], relation);
        }
    };
}
