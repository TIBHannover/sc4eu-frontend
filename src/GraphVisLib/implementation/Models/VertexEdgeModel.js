...
export default class VertexEdgeModel extends BaseModel {
    ...
    addVertex = vertex => {
        if (!this.__isInMap__(this.vertexeMap, this.__resourceIdentifier__(vertex))) {
            this.__integrateVertexEdgeItem(this.modelAsJsonObject.vertices, this.vertexeMap, vertex);
        }
    };

    addEdge = edge => {
        if (!this.__isInMap__(this.edgeMap, this.__resourceIdentifier__(edge))) {
            this.__integrateVertexEdgeItem(this.modelAsJsonObject.edges, this.edgeMap, edge);
        }
    };
