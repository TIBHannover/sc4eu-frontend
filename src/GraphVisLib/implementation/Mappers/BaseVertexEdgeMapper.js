...
export default class BaseVertexEdgeMapper extends BaseComponent {
    constructor() {
        ...
        this.definitionMap = {
            ...
            vertexMapper: {
                __vertexType: 'type', // how do we handle multiple vertex type? << we dont node link model does!
                __vertexEdgeIdentifier: 'resourceURI',
                __displayName: 'annotations.rdfs:label' // will fetch the full object with language defs! node link model gets prefLanguage tag
            },

            edgeMapper: {
                __edgeType: 'type', // how do we handle multiple vertex type? << we dont node link model does!
                __vertexEdgeIdentifier: 'resourceURI',
                __displayName: 'annotations.rdfs:label', // will fetch the full object with language defs! node link model gets prefLanguage tag
    ...
    setInputData = inputDataAsJsonObject => {
        ...
        if (this.requestedInputType === inputDataAsJsonObject.resultingModel.type) {
            ...
            console.error('INPUT MODEL IS OF TYPE ' + inputDataAsJsonObject.resultingModel.type + 'EXPECTED: ' + this.requestedInputType);
        }
    };
    ...
    __mapResourcesToVertex(model, dataArray) {
        ...
        vertexList.forEach(vertex => {
            ...
            if (vertex.resourceReference.axioms && Object.keys(vertex.resourceReference.axioms).length > 0) {
                const axiomsArray = vertex.resourceReference.axioms;
                    ...
                    if (axiomsArray.hasOwnProperty(name)) {
                        ...
                        pAx.forEach(axiom => {
                            if (!model.vertexInMap(axiom)) {
                                console.error('VERTEX DOES NOT EXIST IN THE MAP: TODO CREATE ONE! ');
                            }
                            const targetVertex = model.getVertexFromName(axiom);
                            const axiomEdge = this.__createAxiomEdge(vertex, name, targetVertex);
                            if (axiomEdge) {
