...
export default class NodeLinkMapper extends BaseComponent {
    ...
    _splitNodes(model) {
        ...
        linksToSplit.forEach(link => {
            ...
            newNode.__nodeLinkIdentifier = newNode.__nodeLinkIdentifier + '$$Clone' + cloneIterator;
    ...
    _mapEdgesToLinks(model, dataArray) {
        ...
        dataArray.forEach(item => {
            ...
            const src_node = model.getNodeFromName(item.__source.__vertexEdgeIdentifier);
            const tar_node = model.getNodeFromName(item.__target.__vertexEdgeIdentifier);
            if (src_node) {
