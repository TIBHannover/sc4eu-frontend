...
export default class DrawTools {
    ...
    getMorphParameters(renderingConfig, node) {
        const shapeConfig = renderingConfig.style;
        // shape
        const baseShapeParameters = this.baseShapeMorphParameters(shapeConfig);
        // style
        const shapeStyleParameters = this.shapeStyleMorphParameters(shapeConfig);
        // adjustments based on notation
        this.additionalStyleMorphParameters(baseShapeParameters, node.displayName(), renderingConfig, node);
    ...
    renderLink(groupContainer, propertyContainer, arrowContainer, renderingConfig, link) {
        const renderingLine = this.__createLinkLine(groupContainer, renderingConfig);

        let renderingShape = null;
        let renderingText = null;
        let propertyNodeContainer = null;
        // we dont care about the arrows, they handle them selfs;
        if (renderingConfig.options.drawArrowHead) {
            this.__drawArrowHead(groupContainer, arrowContainer, 'arrowHead_' + link.id(), renderingConfig.style.arrowHead);
        }
        ...
        return {
            ...
            propertyNodeContainer: propertyNodeContainer
        };
    }
    ...
    __createRenderingShapeForPropertyNode(propertyContainer, groupContainer, cfg, link) {
        ...
        return {
            ...
            propertyNodeContainer: groupContainer
        };
    }
    ...
    __drawArrowTail(parent, container, identifier, cfg) {
        this.__drawArrowPrimitive(parent, container, identifier, cfg, 'marker-start');
    }
    ...
    updateLinePosition(link, line, src, tar, linkType, curve) {
        ...
        if (curve === false) {
            const controlPoints = this.lineTools.computeIntersectionPoints(src, tar, 1);
            line.attr('d', this.__directLineFunction(controlPoints));
        } else {
            const controlPoints = this.lineTools.computeIntersectionPointsWithProperty(src, tar, link, 1);
            if (linkType === 'loop') {
        ...
        if (shape) {
            ...
            if (parentGroup) {
                if (curve === false) {
                    ...
                    const renderingNode = d3.select(parentGroup).data()[0];
                    renderingNode.setPosition(centerPoint.cx, centerPoint.cy);

                    renderingNode.x = centerPoint.cx;
                    renderingNode.y = centerPoint.cy;
                    renderingNode.px = centerPoint.cx;
                    renderingNode.py = centerPoint.cy;
    ...
    textMorphParameters = (shape, config, node) => {
        const textMorphParameters = {};
        textMorphParameters[
            'style'
        ] = `font-family: ${config.fontStyle.fontFamily}; font-color: ${config.fontStyle.fontColor};  font-size: ${config.fontStyle.fontSize}`;
    ...
    additionalStyleMorphParameters = (shape, label, config, node) => {
    ...
    applyAdditionalOptions = (group, shape, label, config, node) => {
        ...
        if (
            ...
            const labelBBHeight = this.measureTextHeight(label.text(), config.fontStyle.fontFamily, config.fontStyle.fontSize);
            // this is height align CENTER;
            // it sets the center of the labelBB into the center (0,0) of the rendering group;
            const offset = -0.5 * height + labelBBHeight;
            ...
            nestedGroup.classed('nestedGroupItem', true);
            ...
            nestedLinks.forEach(link => {
                ...
                const linkCFG = this.renderingConfigHandler.getLinkConfigFromType(link.__linkType);

                const nodeCFG = this.renderingConfigHandler.getNodeConfigFromType(link.__target.__nodeType);

                this.renderShadowNode(subGroupProperty, linkCFG.style.propertyNode, link);

                // shift to left side;
                subGroupProperty.attr('transform', 'translate(' + -55 + ',' + (shift + offsetVal) + ')');

                this.renderShadowNode(subGroupNode, nodeCFG, link.__target);

                subGroupNode.attr('transform', 'translate(' + 55 + ',' + (shift + offsetVal) + ')');
    ...
    shapeStyleMorphParameters = renderingConfig => {
        const shapeStyleParameters = {};
        shapeStyleParameters['fill'] = renderingConfig.bgColor;
        const color = renderingConfig['strokeColor'];
        const width = renderingConfig['strokeWidth'];
        const style = renderingConfig['strokeStyle'];

        shapeStyleParameters['stroke'] = color;
        shapeStyleParameters['stroke-width'] = width;
        if (style !== 'solid') {
            if (style === 'dashed') {
                shapeStyleParameters['stroke-dasharray'] = 8;
            }
            if (style === 'dotted') {
                shapeStyleParameters['stroke-dasharray'] = 3;
            }
        }
        return shapeStyleParameters;
    };
    ...
    baseShapeMorphParameters = renderingConfig => {
        ...
        if (renderingConfig.renderingType === 'circle') {
            ...
            baseShapeParameters['y'] = -radius;
            baseShapeParameters['width'] = 2 * radius;
            baseShapeParameters['height'] = 2 * radius;
            baseShapeParameters['rx'] = radius;
            baseShapeParameters['ry'] = radius;
        }

        if (renderingConfig.renderingType === 'rect') {
            baseShapeParameters['x'] = -0.5 * width;
            baseShapeParameters['y'] = -0.5 * height;
            baseShapeParameters['width'] = width;
        ...
        if (renderingConfig.renderingType === 'ellipse') {
            ...
            baseShapeParameters['width'] = width;
            baseShapeParameters['height'] = height;
            baseShapeParameters['rx'] = width;
            baseShapeParameters['ry'] = height;
        }

        return baseShapeParameters;
    };
    ...
    cropText(input, config, width) {
        ...
        while (true) {
            textWidth = this.measureTextWidth(truncatedText, config.fontFamily, config.fontSize);
            if (textWidth <= width) {
                break;
            }
