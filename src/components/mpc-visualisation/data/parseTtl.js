...
const META_PROPERTY_TYPES = new Set([
  RDF_PROPERTY,
  OWL_OBJECT_PROPERTY,
...
function assignRoles(nodeMap, subclassParentsByChild, rootIds = DEFAULT_ROOT_IDS) {
  ...
  if (rootClassIds.size === 0) {
    ...
    rootClassIds = new Set(
      [...nodeMap.keys()].filter(
        (id) =>
          nodeMap.get(id).isClass &&
          !subclassParentsByChild.has(id) &&
          nodesWithChildren.has(id)
      )
    );
  }
  ...
  function resolveDepth(nodeId, visiting = new Set()) {
    ...
    const depth =
      parentIds.length === 0
        ? 99
        : Math.min(...parentIds.map((p) => resolveDepth(p, visiting))) + 1;
...
const TECH_NODE_BUCKETS = [
  ...
  },
  {
    ...
    label: "10–28nm",
    aliases: [
      "10nm_to_less_than_28nm",
      "10nm_to_%3C28nm",
      "TechCategory_10nm_to_%3C28nm",
      "OrderCancellationChange_Semiconductor_10nm_to_lt28nm",
    ],
  },
  {
    ...
    label: "28–45nm",
    aliases: [
      "28nm_to_less_than_45nm",
      "28nm_to_%3C45nm",
      "TechCategory_28nm_to_%3C45nm",
      "OrderCancellationChange_Semiconductor_28nm_to_lt45nm",
    ],
  },
  {
    ...
    aliases: [
      "55nm_to_180nm",
      "TechCategory_55nm_to_180nm",
      "OrderCancellationChange_Semiconductor_55nm_to_180nm",
    ],
  },
  {
    ...
    aliases: [
      "180nm_or_greater",
      "TechCategory_180nm_or_greater",
      "OrderCancellationChange_Semiconductor_180nm_or_greater",
    ],
  },
];

const NODE_ALIASES = Object.fromEntries(
  TECH_NODE_BUCKETS.flatMap((bucket) => bucket.aliases.map((aliasId) => [aliasId, bucket.canonicalId]))
);
...
const TIER_NODE_IDS = {
  OEM_Survey: new Set([
    "Survey", "OEM_Survey",
    "Demand", "CurrentDemandAnalysis", "FutureDemandAnalysis", "DemandForRegion",
    "VehicleType", "BEHV", "BEV", "ICE",
    "AutonomousDrivingDevelopment",
    "TimePeriod", "Quarter",
    "Region", "SemiconductorShortage",
  ]),
  Semiconductor_Survey: new Set([
    "Survey", "Semiconductor_Survey",
    "Demand", "CurrentDemandAnalysis", "FutureDemandAnalysis",
    "TechNode_lte_7nm", "TechNode_10_to_28nm", "TechNode_28_to_45nm",
    "TechNode_55_to_180nm", "TechNode_gte_180nm",
    "Inventory", "InventoryDevelopment", "InventoryTargetIndicator",
    "OrderCancellation", "SemiconductorShortage",
    "Region",
  ]),
...
export async function parseTtlSchema(ttlText) {
  ...
  function makeNode(id) {
    ...
    const role = (overrides.groupKey && overrides.role === "sub")
      ? "sub_clickable"
      : overrides.role;
    return {
      id,
  ...
  for (const surveyId of SURVEY_IDS) {
    ...
    const tierNodes = [...SCHEMA_ALLOWLIST.keys()]
      .filter((id) => tierNodeIds.has(id) && nodeMap.has(id))
      .map(makeNode);
  ...
  const cleanEdges = [...subEdges, ...syntheticEdges]
    .map((e, i) => ({ ...e, id: `edge-${i}` }));
...
export function extractSurveyDomains(schema) {
  ...
  schema.edges.forEach((e) => {
    if (e.style === "sub") {
      ...
      const classes = classByInstanceId.get(e.s) ?? []; classes.push(e.t); classByInstanceId.set(e.s, classes);
    } else if (e.style === "prop") {
      const targets = propTargetsBySource.get(e.s) ?? []; targets.push(e.t); propTargetsBySource.set(e.s, targets);
    }
  });
