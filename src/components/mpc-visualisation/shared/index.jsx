...
import { Box, Typography, Paper, Chip, ListItem, useTheme } from '@mui/material';
import TrendingUpIcon from '@mui/icons-material/TrendingUp';
import TrendingDownIcon from '@mui/icons-material/TrendingDown';
import RemoveIcon from '@mui/icons-material/Remove';
import { useThemePalette, sentimentColors, selectionColors, graphAccents, shadows } from '../config/theme';
...
export const MetaItem = memo(function MetaItem({ label, value }) {
    ...
    return (
        <ListItem disablePadding sx={{ mb: 1.5, flexDirection: 'column', alignItems: 'flex-start' }}>
            <Typography
                sx={{
            ...
            <Typography sx={{ fontSize: FONT_SIZE_BODY, color: colorStyled.onSurface, mt: 0.25 }}>{value}</Typography>
        </ListItem>
    );
});

export const LegendDot = memo(function LegendDot({ label, fill, stroke, shape = 'solid' }) {
    ...
    return (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            ...
            />
            <Typography sx={{ fontSize: FONT_SIZE_MICRO, color: theme.palette.text.secondary }}>{label}</Typography>
        </Box>
    );
});

export const NavChip = memo(function NavChip({ label, isActive, accentColor, onClick }) {
    const { colorStyled } = useThemePalette();
    const theme = useTheme();
    const activeStyles = {
...
export const KpiCard = memo(function KpiCard({ label, primaryValue, secondaryLabel, secondaryValue, isSelected, onClick }) {
    const { colorStyled } = useThemePalette();
    const theme = useTheme();
    ...
    return (
        ...
        >
            ...
            {primaryValue != null && (
                <Typography sx={{ fontSize: FONT_SIZE_KPI, fontWeight: 700, color: primaryColor }}>{formatPercent(primaryValue)}</Typography>
            )}
